...
class BST(BinaryTree):
    ...
    def is_bst_satisfied(self):
        ...
        whether insert/delete functions are actually working.
        '''
        if self.root:
    ...
    def _is_bst_satisfied(node):
        ...
        are less than/greater than,rather than ensuring that
        all nodes to the left/right are less than/greater than.
    ...
    def _remove(node, value):
        ...
        if node.value > value:
            ...
            if node.left is None:
                ...
                return curr
            # 2 children
            curr = BST._find_smallest(node.right)
            node.value = curr
            node.right = BST._remove(node.right, curr)
        return node
